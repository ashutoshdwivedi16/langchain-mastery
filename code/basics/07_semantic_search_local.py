"""
Feature 7: Advanced Semantic Search Patterns - LOCAL VERSION
Covers:
  A. Basic similarity search vs semantic search
  B. MultiQueryRetriever  — ask the same question multiple ways
  C. ContextualCompressionRetriever — compress retrieved docs before sending to LLM
  D. Ensemble Retriever — combine keyword + semantic search (BM25 + FAISS)

100% local — Ollama + HuggingFace embeddings + FAISS. No API key needed.
"""
...
DOCUMENTS = [
    ...
    Document(page_content="Ollama allows you to run large language models locally on your machine without any API key or internet connection.", metadata={"source": "ollama_intro", "topic": "ollama", "year": 2023}),
...
def basic_similarity_search(vectorstore):
    ...
    queries = [
        "How do I run AI models for free?",
...
def similarity_score_search(vectorstore):
    ...
    query = "local LLM without internet"
...
def contextual_compression_retriever(vectorstore, llm):
    ...
    try:
        ...
        print(f"  Compression error (local LLM may not follow format): {e}")
...
def advanced_rag_pipeline(vectorstore, llm):
    ...
    questions = [
        ...
        "How can I run LLMs without an API key?",
...
def main():
    ...
    print("\n💡 Feature 7: Advanced Semantic Search (100% Local!)")
